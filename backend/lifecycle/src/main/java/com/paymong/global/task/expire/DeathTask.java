...
public class DeathTask implements BasicTask {
    ...
    public void afterStop(Long mongId, BasicScheduler basicScheduler) throws NotFoundFailException {
        ...
        // 죽음 처리
        mong.setStatusCode(MongConditionCode.DIE.code);
        mong.setActive(false);

        // 죽음 로그 저장
        mongHistoryRepository.save(MongHistory.builder()
                .mongId(mongId)
                .code(MongConditionCode.DIE.code)
                .build());

        BasicTask.writeLog(mongId, TaskMessage.DEATH);
    }
}
