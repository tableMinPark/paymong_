package com.paymong.pay_point.global.security;
...
public class TokenExceptionFilter extends GenericFilterBean {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse response, FilterChain chain) {
        ...
        try {
            ...
            log.info("TokenExceptionFilter : 인증/인가에 필요한 정보가 없음 : {}", internalToken);
            setFailResponse((HttpServletResponse) response, e.getFailCode());
        } catch (Exception e) {
            e.printStackTrace();
            String internalToken = request.getHeader("InternalToken");
            log.info("TokenExceptionFilter : 알 수 없는 에러 발생, {}", internalToken);
